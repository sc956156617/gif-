...
@interface SCLoop () <UIScrollViewDelegate>
//轮播的图片数组
...
@implementation SCLoop

#pragma mark - 初始化方法
//创建用来缓存图片的文件夹
+ (void)initialize {
    cache = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"SCLoop"];
    BOOL isDir = NO;
    ...
    for (int i = 0; i < imageArray.count; i++) {
        if ([imageArray[i] isKindOfClass:[UIImage class]]) {
            ...
            if (_placeholderImage) {
                ...
                UIImage *placeholderImage = [UIImage imageNamed:@"SCLoop"];
...
- (void)imageClick {
    ...
    } else if ([self.SCDelegate respondsToSelector:@selector(loopView:clickImageAtIndex:)]){
        [self.SCDelegate loopView:self clickImageAtIndex:self.currIndex];
    }
}
